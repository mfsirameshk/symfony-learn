...
abstract class BaseProductQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'Ramesh\\MyPropelBundle\\Model\\Product', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `name`, `price`, `description`, `category_id` FROM `product` WHERE `id` = :p0';
        try {
    ...
    public function filterByCategory($category, $comparison = null)
    {
        if ($category instanceof Category) {
            ...
            return $this
                ->addUsingAlias(ProductPeer::CATEGORY_ID, $category->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Ramesh\MyPropelBundle\Model\CategoryQuery A secondary query class using the current class as primary query
     */
    public function useCategoryQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinCategory($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Category', '\Ramesh\MyPropelBundle\Model\CategoryQuery');
    }
